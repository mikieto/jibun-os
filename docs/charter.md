...
> **「コピー可能な自己 (The Reproducible Self)」** を構築し、学習と価値創出のループを高速化すること。

その達成のため、以下の **二層＋ハブ層** を統合したフレームを採用する。

| 階層                        | 役割                           | 物理パス                            |
| ------------------------- | ---------------------------- | ------------------------------- |
| **Prompt‑Ingestion OS**   | 直近文脈を AI に高速インジェストし即応判断を最適化  | `records/` , RAM Context        |
| **Digital‑Twin OS**       | 行動・意思決定の履歴を蓄積し長期学習・責任分担を担保   | `constitution/`, `legislation/` |
| **Precedents Layer (L3)** | 短期ログの結晶化知識を保持し、両層へ知見をフィードバック | `precedents/`                   |
| **Jibun OS (Unified)**    | 各層を橋渡しし自己進化ループを駆動            | 全ディレクトリ                         |

---
## 第一条：目的（Why） ― デジタルツイン × プロンプトインジェスション

本システムの唯一かつ最高の目的は、 **「デジタルツイン（長期記憶）」と「プロンプトインジェスション（短期文脈）」を統合運用しながら、自己を探求・拡張し続けること** である。
